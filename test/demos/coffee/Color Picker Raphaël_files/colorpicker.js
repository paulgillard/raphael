...
(function (Raphael) {
    ...
    Raphael.fn.colorPickerIcon = function (x, y, r) {
        ...
        for (var i = 0; i < segments; i++) {
          a = "hsb(" + (segments - i) * (360 / segments) + ", 1, 1)";
          hsb = new Colour(a);
            this.path(path).attr({
                stroke: "none",
                fill: "hsb(" + (segments - i) * (360 / segments) + ", 1, 1)",
    ...
    var doc = document, win = window,
        ColorPicker = function (x, y, size, initcolor, element) {
            ...
            t.cursor = r.set();
            a = r.circle(size2, size2, size20 / 2).attr({
                stroke: "#000",
                opacity: .5,
                "stroke-width": w3
            });
    ...
    ColorPicker.prototype.setHS = function (x, y) {
        ...
        this.H = (1 - d / 360) % 1;
        this.S = Math.min((X * X + Y * Y) / R / R, 1) / 100;
        this.brect.attr({fill: "180-hsl(" + [this.H, this.S] + ",0.01)-#000"});
        this.onchange && this.onchange(this.color());
    };
    ...
    ColorPicker.prototype.color = function (color) {
        if (color) {
            color = new Colour(color).toRGB();
            var hex = color.hex();
            color = color.toHSB();
            d = color.hue * 360;
            this.H = color.hue;
            this.S = color.saturation;
            this.B = color.brightness;
            ...
            return new HSB(this.H, this.S, this.B).toRGB().hex();
        }
    };
})(window.Raphael);