...
/**/    R.version = "1.4.7";
        var separator = /[, ]+/,
            ...
            oldRaphael = {
                was: window.hasOwnProperty("Raphael"),
                is: window.Raphael
            },
/**/        Paper = function () {},
...
/**/                String.prototype.toLowerCase.call(Object.prototype.toString.call(o).slice(8, -1)) == type;
/**/    };
...
/**/                var bod;
/**/                color = String(color).replace(trim, E);
/**/                try {
/**/                    var docum = new window.ActiveXObject("htmlfile");
/**/                    docum.write("<body>");
/**/                    docum.close();
/**/                    bod = docum.body;
/**/                } catch(e) {
...
/**/            toHex = cacher(function (color) {
/**/                i.style.color = color;
/**/                return document.defaultView.getComputedStyle(i, E).getPropertyValue("color");
/**/            });
/**/        }
/**/        return toHex(color);
/**/    },
...
/**/        }
/**/        !(hsrg.hasOwnProperty(colour.substring(0, 2)) || colour.charAt() == "#") && (colour = toHex(colour));
/**/        var res,
/**/            red,
/**/            green,
/**/            blue,
...
/**/            b = b.replace(rg, "0");
/**/            rgb.hex = "#" + r + g + b;
/**/            isFinite(parseFloat(opacity)) && (rgb.o = opacity);
/**/            return rgb;
/**/        }
/**/        return {r: -1, g: -1, b: -1, hex: "none", error: 1};
/**/    }, R);
/*NA*/    R.getColor = function (value) {
/*NA*/        var start = this.getColor.start = this.getColor.start || {h: 0, s: 1, b: value || .75},
/*NA*/            rgb = this.hsb2rgb(start.h, start.s, start.b);
/*NA*/        start.h += .075;
/*NA*/        if (start.h > 1) {
/*NA*/            start.h = 0;
/*NA*/            start.s -= .2;
/*NA*/            start.s <= 0 && (this.getColor.start = {h: 0, s: 1, b: start.b});
/*NA*/        }
/*NA*/        return rgb.hex;
/*NA*/    };
/*NA*/    R.getColor.reset = function () {
/*NA*/        delete this.start;
/*NA*/    };
        // path utilities
...
/**/                Y.push(y);
/**/            } else {
/**/                var dim = curveDimensions(x, y, p[1], p[2], p[3], p[4], p[5], p[6]);
...
/**/                var r = res[i] = [],
/**/                    pa = pathArray[i];
/**/                if (pa[0] != String.prototype.toLowerCase.call(pa[0])) {
/**/                    r[0] = String.prototype.toLowerCase.call(pa[0]);
/**/                    switch (r[0]) {
/**/                        case "a":
/**/                            r[1] = pa[1];
/**/                            r[2] = pa[2];
...
/**/            };
/**/        }),
/**/        pathToCurve = cacher(function (path, path2) {
...
/**/                            path = ["C"].concat(quadraticToCurve(d.x, d.y, d.qx, d.qy, path[1], path[2]));
/**/                            break;
/**/                        case "Q":
/**/                            d.qx = path[1];
/**/                            d.qy = path[2];
/**/                            path = ["C"].concat(quadraticToCurve(d.x, d.y, path[1], path[2], path[3], path[4]));
/**/                            break;
/**/                        case "L":
...
/**/    // SVG
/**/    if (R.svg) {
/**/        Paper.prototype.svgns = "http://www.w3.org/2000/svg";
/**/        Paper.prototype.xlink = "http://www.w3.org/1999/xlink";
/**/        round = function (num) {
...
/**/                }
/**/            } else {
/**/                el = document.createElementNS(Paper.prototype.svgns, el);
...
/**/            id = id.match(/^url\(#(.*)\)$/);
/**/            id && SVG.defs.removeChild(document.getElementById(id[1]));
/**/            
...
/**/        };
        ...
        if (R.vml) {
            var map = {M: "m", L: "l", C: "c", Z: "x", m: "t", l: "r", c: "v", z: "x"},
                ...
                path2vml = function (path) {
                    ...
                    String(path).match(total) && (command = pathToCurve);
                    total = /[clmz]/g;
                    ...
                    for (var i = 0, ii = pa.length; i < ii; i++) {
                        ...
                        for (var j = 1, jj = p.length; j < jj; j++) {
                            r += Math.round(p[j] * zoom) + (j != jj - 1 ? "," : E);
                        }
                        res.push(r);
                    }
                    return res.join(S);
            ...
            setFillAndStroke = function (o, params) {
                ...
                params["font-family"] && (s.fontFamily = '"' + params["font-family"].split(",")[0].replace(/^['"]+|['"]+$/g, E) + '"');
                ...
                if (res.type == "text") {
                    ...
                    res.node.string && (res.paper.span.innerHTML = String(res.node.string).replace(/</g, "&#60;").replace(/&/g, "&#38;").replace(/\n/g, "<br>"));
            ...
            addGradientFill = function (o, gradient) {
                ...
                gradient = String(gradient).replace(radial_gradient, function (all, fx, fy) {
                    ...
                    if (fx && fy) {
                        fx = parseFloat(fx);
                        fy = parseFloat(fy);
                        Math.pow(fx - .5, 2) + Math.pow(fy - .5, 2) > .25 && (fy = Math.sqrt(.25 - Math.pow(fx - .5, 2)) * ((fy > .5) * 2 - 1) + .5);
                        fxfy = fx + S + fy;
                    }
                    return E;
                });
            ...
            Element.prototype.attr = function (name, value) {
                ...
                if (params) {
                    ...
                    (!pathlike.hasOwnProperty(this.type) || this._.rt.deg) && this.setBox(this.attrs);
                }
                return this;
            };
            Element.prototype.toFront = function () {
        ...
        if ((navigator.vendor == "Apple Computer, Inc.") && (navigator.userAgent.match(/Version\/(.*?)\s/)[1] < 4 || window.navigator.platform.slice(0, 2) == "iP")) {
            Paper.prototype.safari = function () {
                ...
                window.setTimeout(function () {rect.remove();});
            };
        } else {
            Paper.prototype.safari = function () {};
        }
        ...
        for (var i = events.length; i--;) {
            (function (eventName) {
                R[eventName] = Element.prototype[eventName] = function (fn) {
                    if (R.is(fn, "function")) {
        ...
        Paper.prototype.setSize = setSize;
        Paper.prototype.top = Paper.prototype.bottom = null;
        Paper.prototype.raphael = R;
        function x_y() {
        ...
        var getPointAtSegmentLength = cacher(function (p1x, p1y, c1x, c1y, c2x, c2y, p2x, p2y, length) {
            ...
            for (var i = 0; i < 1.01; i+=.01) {
                ...
                i && (len += Math.pow(Math.pow(old.x - dot.x, 2) + Math.pow(old.y - dot.y, 2), .5));
                if (len >= length) {
                    return dot;
                }
                old = dot;
        ...
        var animationElements = {length : 0},
            animation = function () {
                var Now = +new Date;
                for (var l in animationElements) if (l != "length" && animationElements.hasOwnProperty(l)) {
                ...
                animationElements.length && window.setTimeout(animation);
            },
                ...
                if (availableAnimAttrs.hasOwnProperty(attr)) {
                    ...
                    switch (availableAnimAttrs[attr]) {
                        case "along":
                            ...
                            to.back = params.back;
                            to.len = len;
                            params.rot && (diff.r = parseFloat(this.rotate()) || 0);
                            break;
                        case "number":
                        ...
                        case "colour":
                            ...
                            break;
                        case "path":
                            var pathes = pathToCurve(from[attr], to[attr]);
                            from[attr] = pathes[0];
                            var toPath = pathes[1];
                            diff[attr] = [];
                            for (var i = 0, ii = from[attr].length; i < ii; i++) {
                            ...
                            break;
                        case "csv":
                            var values = String(params[attr]).split(separator),
